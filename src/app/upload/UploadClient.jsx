...
export default function UploadPage({ accessToken }) {
  ...
  const loadBuckets = useCallback(async ({ selectFirst = false } = {}) => {
    try {
      const res  = await fetch(`${API_BASE}/buckets`, { headers: authHeader() });
  ...
  async function executeBucketDelete(name) {
    setIsDeleting(true);
    try {
      const res  = await fetch(`${API_BASE}/buckets/${encodeURIComponent(name)}`, { method: "DELETE", headers: authHeader() });
      const json = await res.json();
  ...
  const loadFiles = useCallback(async (bucket) => {
    try {
      const res  = await fetch(`${API_BASE}/buckets/${encodeURIComponent(bucket)}/objects`, { headers: authHeader() });
      const json = await res.json();
    ...
    for (let i = 0; i < allowed.length; i++) {
      ...
      try {
        const res  = await fetch(
          ...
          { method: "PUT", body: form, headers: authHeader() }
        );
        const json = await res.json();
